...
- **Problem Statement:**
  ...
  2. **Empirical Side-by-Side Forensic Truth (`/auth/login` vs `/auth/register`):**
     ...
     - **Why `/auth/register` Failed with `(no captcha_token found)`:**
       - *Railway Deployment Lag & Monorepo CI Blocker:* While PR #607 and PR #608 merged changes to `master`, the GitHub Actions workflow `CI & Selective Monorepo Verification` failed on `master` due to unrelated issues in `sentinel-core` (SSG prerender missing Supabase build-time env vars, and 11 legacy broken tests). Railway's deployment pipeline has GitHub checks integration enabled ("Wait for CI"), so Railway refused to deploy the updated `sentinel-api` container, leaving production running the historical container where `AuthService.register` dropped `captchaToken`.
       - *Aggressive IP Rate Limit Trap:* `app/sentinel-api/src/modules/identity/auth/auth.routes.ts` enforced `limit: 3, windowSeconds: 3600` on `/auth/register`. A user retrying after errors had their IP (`122.2.100.112`) locked out in Upstash Redis (`rl:auth:register:122.2.100.112 - 3/3 (Remaining: 0)` as confirmed in Railway logs), escalating to HTTP 429 (`Too many requests. Please try again in 3282 seconds.`).
       ...
       - *Error Callback Reset Cycle:* On failure, `onError` resets the widget (`turnstileRef.current?.reset()`), causing the widget to spin through `Verifying...` before displaying `Success!`, while the error banner remains visible.
  3. **Distinction Between Email/Password Registration and Google OAuth Authentication:**
     - *Email/Password Flow (Requires OTP Verification):*
       1. Student submits registration form with email, password, and solved Turnstile CAPTCHA.
       2. Supabase GoTrue creates unconfirmed account and dispatches a 6-digit OTP code to student's inbox.
       3. Browser unconditionally redirects to `/auth/confirm-code?email=...`.
       4. Student inputs the 6-digit code, verifying email ownership.
       5. Upon code verification, session is activated and student is routed to `/onboarding`.
     - *Google OAuth Flow (Inherently Pre-Verified, Bypasses OTP Code Page):*
       1. Student clicks "Continue with Google".
       2. Google authenticates the student and asserts `email_verified: true`.
       3. Browser returns to `/auth/callback?code=...` where Supabase exchanges the OAuth code for an active session.
       4. No 6-digit OTP code is generated by Supabase (because Google already verified email ownership).
       5. `auth/callback` invokes `resolveWebAuthState(supabase, user)`. Since the student has not yet entered their academic credentials (student number, institution, department) into the `students` table, `isFullyOnboarded` evaluates to `false`.
       6. Student is correctly routed directly to `https://app.sentinelph.tech/onboarding` to complete their profile (as captured in Screenshot 2). Routing OAuth users to `/auth/confirm-code` would trap them indefinitely since no OTP code exists.
  4. **Missing Confirmation Code Page & Premature Onboarding Redirect (Historical):** Previously, if `data.session` was returned, the registration hook redirected immediately to `/onboarding`, bypassing confirmation. Furthermore, if `session` was null, it only flipped an in-memory React state (`setStep('verify')`) on `/auth/register` that was wiped on page reload or when navigating away to check email.
...
- **Business / User Value:**
  ...
  - **Strict Security Gate:** Guarantees that every student registering via email/password must verify their email with the 6-digit OTP code before accessing the student onboarding portal, while allowing pre-verified OAuth students to smoothly proceed directly to profile setup.
