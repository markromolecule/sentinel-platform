...
- **Resolved verification gap:** The previous direct-function `QuestionCard` test was replaced with a `react-test-renderer` harness that renders nested React JSX. The targeted session suites now initialize under Vitest with narrow Expo-native setup mocks. `pnpm --filter sentinel-mobile test -- --runInBand` passes 51 files and 366 tests.
...
- **Selected mobile repair:** The implemented mobile-only repair restores `QuestionCard` scroll content growth (`flexGrow: 1`) and footer-safe bottom space (`paddingBottom: 140`). This is covered by a native render regression assertion and keeps API/data/proctoring contracts unchanged.
...
Manual native QA remains pending on an Android emulator/device. The required confirmation is: after entering a populated attempt with proctoring disabled, the first prompt, `QuestionCard` header, and answer control are visible, and the development-only render classification shows a mounted card with non-zero viewport/card layout.
