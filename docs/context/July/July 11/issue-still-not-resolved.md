...
The primary suspect is the **frontend filtering logic**. The user explicitly notes: *"the issue only occurs when the examination is private but assigned to the classroom."* This strongly points to the status-normalization pipeline:
...
#### Point 1 — Telemetry Disabled Globally *(low probability — would also affect local)*
`TelemetryIngestionService.processEvent()` checks settings first:
```ts
if (settingsRecord && !settingsRecord.value.operations.enabled) { return; }
```
...
#### Point 2 — Policy Rule Ignoring Events *(low probability — would also affect local)*
...
#### Point 4 — Session Eligibility Rejection *(medium probability)*
