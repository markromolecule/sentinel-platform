...
- Redesign `TabsList` to use the standard compact layout, replacing custom styling with a fixed-width container:
    ```tsx
    <TabsList className="bg-muted grid h-9 w-[160px] grid-cols-2 p-0.5">
        <TabsTrigger value="active" className="rounded-md text-xs font-semibold">
            Active
        </TabsTrigger>
        <TabsTrigger value="archived" className="rounded-md text-xs font-semibold">
            Archived
        </TabsTrigger>
    </TabsList>
    ```
