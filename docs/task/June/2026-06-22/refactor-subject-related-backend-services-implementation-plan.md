...
*Tradeoff:* High refactoring footprint across the three modules, but fully aligns with the repository's modular service layer standard, makes files readable (<200 lines), and keeps interface backward-compatible.
...
*Tradeoff:* Faster to implement and fewer files created, but leaves the main service files partially cluttered and inconsistent in architecture.
...
*Tradeoff:* Eliminates an extra layer of delegation, but dramatically increases the code changes in controllers and breaks backward compatibility.
