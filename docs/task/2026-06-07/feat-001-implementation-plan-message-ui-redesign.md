...
> [!IMPORTANT]
> **Q1 — Global `<main>` padding removal:** Removing `p-6` from the protected layout `<main>` affects *all* pages, not just messages. Do you want each sibling page to add its own `p-6` wrapper, or should we scope the padding removal only to the messages route (e.g., via a route group or a conditional CSS variable)?
