...
*Tradeoff:* Faster to implement and avoids routing/layout modifications, but clutters the report view and degrades performance for large log volumes.
...
*Tradeoff:* Provides a clean, highly structured workspace and premium UX, but requires modifying routing structure for existing exam sub-pages.
...
*Tradeoff:* Keeps exam management simple, but introduces disjointed user navigation and higher authentication overhead.
