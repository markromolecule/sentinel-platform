...
3. **Existing rich text editor.** Is an editor library already a dependency in either
   app? If yes, reuse it. If introducing one fresh, it must be the *same* library,
...
6. Sanitize HTML before rendering on *every* student- or instructor-facing surface
...
- [ ] Manually verify:
  ...
  - [ ] rendering images and code blocks in exam preview *and* the review/results screen
