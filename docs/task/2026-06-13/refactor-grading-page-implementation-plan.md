...
- Define default configuration constants like `GRADING_ATTEMPT_QUERY_KEY` and fallback scores.
...
- Define `CriteriaScores`, `QuestionEvaluationState`, `EvaluationsState`, and `ScoreSummary` types.
...
- Define types specific to the parameters and return contract of `useGradingAttempt`.
...
- Implement `useGradingAttempt` to encapsulate grading query, mutation, local states, and handlers.
...
- Unit test suite for `useGradingAttempt` to verify initial states, score calculation, and mutations.
...
- Define properties for all sub-components.
...
- Renders the grading error card.
...
- Renders page header with back button and grading submit action.
...
- Renders summary metrics cards (Objective, Essay, Final scores, attempt status).
...
- Renders the left pane containing essay prompt, prompt switcher, student answer, and feedback text area.
...
- Renders the right pane with criteria sliders (0-4) and overall feedback text area.
...
- Clean up file, utilizing hook and component modular imports.

## Verification Plan

### Automated Tests

- Run `pnpm --dir app/sentinel-web test` to verify Vitest tests.

### Manual Verification

- Render page and ensure UI/UX matches the original state exactly and functions correctly.
