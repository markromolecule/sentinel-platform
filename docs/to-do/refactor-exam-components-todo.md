...
- *Pros*: Highly decoupled features; all files for a single business capability live together.
- *Cons*: Doesn't clearly separate UI primitives (a form vs. a dialog vs. a view), leading to mental overhead when searching for generic component types.
...
- *Pros*: Completely eliminates deep nesting. Aligns with standard component organization (Dialogs go in `dialogs/`, Views in `views/`). Highly predictable.
- *Cons*: Splits features apart (e.g., the `exam-assign-dialog` lives far from the `useExamAssignment` hook if we move hooks, though hooks typically go to `_hooks/`).
