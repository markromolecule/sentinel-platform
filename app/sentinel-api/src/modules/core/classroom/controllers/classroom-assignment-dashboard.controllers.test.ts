...
describe('Classroom Assignment Dashboard Controllers', () => {
    ...
    describe('GET /dashboard/unassigned (getUnassignedClassrooms)', () => {
        it('returns unassigned classrooms list', async () => {
            ...
            vi.mocked(ClassroomService.getUnassignedClassrooms).mockResolvedValue(
                mockClassrooms as any,
            );
    ...
    describe('GET /dashboard/instructor-loads (getInstructorLoadSummary)', () => {
        it('returns instructor load list', async () => {
            ...
            vi.mocked(ClassroomService.getInstructorLoadSummary).mockResolvedValue(
                mockLoads as any,
            );

            const res = await app.request(
                '/dashboard/instructor-loads?termId=11111111-1111-4111-8111-111111111111',
            );

            expect(res.status).toBe(200);
            expect(ClassroomService.getInstructorLoadSummary).toHaveBeenCalledWith(
                expect.any(Object),
                {
    ...
    describe('GET /:id/suggestions (getSmartSuggestions)', () => {
        it('returns ranked recommendations', async () => {
            ...
            vi.mocked(ClassroomService.getSmartSuggestions).mockResolvedValue(
    ...
    describe('POST /dashboard/bulk-assign (bulkAssignClassroomInstructors)', () => {
        it('successfully triggers bulk assignments', async () => {
            ...
            vi.mocked(ClassroomService.bulkAssignClassroomInstructors).mockResolvedValue(
                mockResults as any,
            );
            ...
            expect(res.status).toBe(200);
            expect(ClassroomService.bulkAssignClassroomInstructors).toHaveBeenCalledWith(
                expect.any(Object),
                {
                    assignments: [
                        {
