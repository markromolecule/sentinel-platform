...
export class RoomService {
    ...
    static async bulkCreateRooms(
        ...
        if (!targetInstitutionId || targetInstitutionId === '') {
            console.error(
                `Attempted to bulk create rooms for user ${createdBy} without an institutionId`,
            );
            throw new HTTPException(403, {
                message:
                    'Your account is not associated with an institution. Please contact your administrator.',
        ...
        try {
            ...
            const institutionName = await this.getInstitutionName(dbClient, targetInstitutionId);
