...
export function buildStudentWhitelistQuery(dbClient: DbClient) {
    return dbClient
        ...
        .leftJoin('user_profiles as claimed_profile', 'claimed_profile.user_id', 'sw.claimed_user_id')
        .select([
            'sw.whitelist_id',
            'sw.institution_id',
            ...
            sql<string | null>`NULLIF(TRIM(CONCAT_WS(' ', claimed_profile.first_name, claimed_profile.last_name)), '')`.as(
                'claimed_name',
            ),
        ]);
}
