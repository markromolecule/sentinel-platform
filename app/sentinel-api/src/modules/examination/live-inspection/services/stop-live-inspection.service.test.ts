...
describe('stopLiveInspection', () => {
    ...
    it('returns terminal lease immediately without cleanup or update', async () => {
        ...
        vi.mocked(repository.getLiveInspectionLeaseForViewer).mockResolvedValue(
            terminalLease as any,
        );
    ...
    it('moves REQUESTED lease to STOPPING, performs cleanup, terminalizes, logs lifecycle, and returns ENDED', async () => {
        ...
        const endedLease = {
            ...
            state: 'ENDED',
            version: 3,
            end_reason: 'VIEWER_STOPPED',
        };

        vi.mocked(repository.getLiveInspectionLeaseForViewer).mockResolvedValue(
            requestedLease as any,
        );
        vi.mocked(repository.compareAndSetLiveInspectionLeaseState).mockResolvedValue(
            stoppingLease as any,
        );
        vi.mocked(stateService.terminalizeLiveInspectionLeaseState).mockResolvedValue(
            endedLease as any,
        );
    ...
    it('throws 409 if convergence retries are exhausted due to continuous conflicts', async () => {
        ...
        await expect(
            ...
            new HTTPException(409, { message: 'Live inspection lease state transition conflict.' }),
        );
    });
});
