...
import { Alert, AlertDescription, AlertTitle, Badge, Button, cn } from '@sentinel/ui';
...
export function TelemetrySettingsForm({
    ...
    return (
        <div className="relative flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-16">
            <SettingsNav activeSection={activeSection} onActiveSectionChange={setActiveSection} />
                    ...
                    <div className="space-y-3">
                        {warnings.map((warning) => (
                            ...
                            >
                                ...
                                <AlertTitle className="ml-2 text-sm font-semibold text-amber-600">
                                    {warning.title}
                                </AlertTitle>
            ...
            {/* Sticky Action Bar */}
            <div className="bg-background/80 fixed right-0 bottom-0 left-0 z-50 border-t px-6 py-4 backdrop-blur-md lg:left-0">
                <div className="mx-auto flex max-w-7xl items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <Badge
                            variant={isDirty ? 'default' : 'secondary'}
                            className={cn(
                                'px-2.5 py-0.5 text-[10px] font-bold tracking-wider uppercase',
                                isDirty
                        ...
                        <p className="text-muted-foreground hidden text-[11px] font-medium sm:block">
                            Runtime schema v{currentDraft.version} • Full-replace semantics applied
                            on save.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <Button
                            ...
                            className="hover:bg-destructive/10 hover:text-destructive text-xs font-semibold transition-colors"
                        >
                            Discard Changes
                        </Button>
                        <Button
