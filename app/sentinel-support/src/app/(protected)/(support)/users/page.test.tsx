...
vi.mock('@/app/(protected)/(support)/users/_components', () => ({
    AddAdminDialog: ({ role }: { role: string }) => <div>invite-dialog:{role}</div>,
...
describe('SupportUsersPage', () => {
    beforeEach(() => {
        ...
        mockUseUsersQuery.mockImplementation(({ role }: { role: string }) => ({
            data: [],
            isLoading: role === 'support',
            error: null,
        }));
    });
