...
/**
 * Renders compact lifecycle and score-state cues for one monitored attempt.
 */
export function AttemptLifecycleBadge({ student }: { student: StudentSession }) {
    return (
        <>
            <Badge
                variant="outline"
                className={`px-1.5 py-0.5 text-[10px] ${getLifecycleBadgeClassName(student)}`}
            >
            ...
            {student.scoreState ? (
                <Badge
                    variant="outline"
                    className="border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] text-slate-700"
                >
