...
export function isQuestionComplete(type: QuestionType, content: ExamQuestionContent): boolean {
    ...
    switch (type) {
        case "MULTIPLE_CHOICE": {
            ...
            if (typeof content.correctAnswer !== "string" || !content.correctAnswer.trim()) return false;
        ...
        case "MULTIPLE_RESPONSE": {
            ...
            const answers = content.correctAnswer.filter((answer) => !!answer.trim());
        ...
        case "MATCHING": {
            ...
            return pairs.length > 0 && pairs.every((pair) => !!pair.left.trim() && !!pair.right.trim());
        }
        case "FILL_BLANK": {
