...
vi.mock('@sentinel/ui', () => ({
    ...
    DataTable: ({ columns, data, onRowClick, meta }: any) => {
        return (
                    ...
                    <tr>
                        {columns.map((col: any, idx: number) => {
                            const headerContent = typeof col.header === 'function'
                                ...
                                : (col.header || '');
                            return <th key={col.id || col.accessorKey || idx}>{headerContent}</th>;
                        })}
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr key={row.incidentId || rowIdx} onClick={() => onRowClick?.(row)}>
                            {columns.map((col: any, colIdx: number) => {
                                const cellContent = typeof col.cell === 'function'
                                    ...
                                    : (row[col.accessorKey] || '');
                                return <td key={col.id || col.accessorKey || colIdx}>{cellContent}</td>;
                            })}
                        </tr>
                    ))}
                </tbody>
...
describe('ExamIncidentLogsPage Component', () => {
    ...
    it('renders empty state when no exam is selected', async () => {
        ...
        expect(screen.getByText('Select an Examination')).toBeTruthy();
