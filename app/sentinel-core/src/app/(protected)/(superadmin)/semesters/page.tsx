"use client";

import { useDebounce, useSemestersQuery } from "@sentinel/hooks";
import { useState } from "react";
import { AddSemesterDialog, SemestersList } from "@/app/(protected)/(superadmin)/semesters/_components";
import { PageHeader, Separator } from "@sentinel/ui";

// superadmin semesters page
export default function SuperadminSemestersPage() {
    const [searchTerm, setSearchTerm] = useState("");
    const debouncedSearch = useDebounce(searchTerm, 500);

    // get semesters from the api
    const { data: semesters = [], isLoading, isError } = useSemestersQuery(debouncedSearch);

    return (
        <div className="flex flex-col gap-6 md:p-6 p-4">
            <PageHeader
                title="Semester Management"
                description="Manage academic semesters, terms, and their schedules."
            >
                <AddSemesterDialog />
            </PageHeader>
            <Separator />

            <div className="relative">
                <SemestersList
                    semesters={semesters}
                    searchTerm={searchTerm}
                    onSearchChange={setSearchTerm}
                    isLoading={isLoading}
                />

                {/* Subtle loading overlay only for initial empty state */}
                {isLoading && semesters.length === 0 && (
                    <div className="absolute inset-x-0 bottom-0 top-[60px] flex items-center justify-center bg-background/80 z-10 rounded-md">
                        <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
                    </div>
                )}

                {isError && (
                    <div className="mt-4 flex h-32 items-center justify-center text-destructive bg-destructive/5 rounded-md border border-destructive/20">
                        Error loading semesters. Please try again.
                    </div>
                )}
            </div>
        </div>
    );
}
