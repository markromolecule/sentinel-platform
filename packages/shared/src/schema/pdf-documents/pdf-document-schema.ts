 ...
 */
export const LifecycleStatusSchema = z.enum(['PENDING', 'GENERATING', 'READY', 'FAILED', 'EXPIRED']);
...
export const HeaderConfigSchema = z.object({
    ...
    divider_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Must be a valid hex color').default('#D1D5DB'),
    accent_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Must be a valid hex color').default('#3B82F6'),
...
export const FooterConfigSchema = z.object({
    ...
    divider_visible: z.boolean().default(true),
    divider_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Must be a valid hex color').default('#E5E7EB'),
...
export const ReportPeriodSchema = z.object({
    preset: z.enum(['LAST_7_DAYS', 'LAST_30_DAYS', 'LAST_90_DAYS', 'CUSTOM']).default('LAST_30_DAYS'),
    start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Must be in YYYY-MM-DD format').optional().nullable(),
